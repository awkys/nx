...
class AdminController extends UserController
{
    ...
    {
        $logs = Manager::connection()->select('SELECT *,(SUM(u) + SUM(d)) as sum_ud FROM `user_traffic_log` where unix_timestamp(date_sub(now(), interval 1 hour)) < log_time GROUP BY node_id ORDER BY node_id LIMIT 0,20');
        foreach ($logs as &$log) {
