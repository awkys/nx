...
class MetronPay extends AbstractPayment
{
    public function purchase($request, $response, $args, $telegram = 0)
    {
        ...
        if ($type == 'alipay' || $type =='pay_alipay') {
            # 支付宝
        ...
        } else if ($type == 'wxpay' || $type == 'pay_wxpay') {
            # 微信支付
    ...
    public function notify($request, $response, $args)
    {
       ...
       // file_put_contents(BASE_PATH . '/storage/pay.log', $payment_system . "\r\n", FILE_APPEND);
        switch ($payment_system) {
            case ('wolfpay'):
...
//同步回调
    public function getReturnHTML($request, $response, $args)
    {
       ...
       // file_put_contents(BASE_PATH . '/storage/easypay.log', $_GET['status'], FILE_APPEND);
        if ($p->status === 1) {      # 充值已完成
            $result['status'] = 1;
            # 记录中商品字段存在
            if ($p->shop) {
