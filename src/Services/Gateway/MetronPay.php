        ...
        if ($type == 'pay_alipay') {
            ...
            switch ($payment_system) {
                ...
                case ('tomatopay'):
                    ...
                    $result = $tomatopay->MetronPay($type, $price, $shopinfo, $paylist_id);
                    if ($result['ret'] === 1) {
                        $return = array(
                            'ret' => 1,
                            'type' => 'url',
        ...
        switch ($payment_system) {
            ...
            case ('tomatopay'):
                ...
                if ($sign == $signs) {
                    ...
                    $money = $p->total;
                    if ($p->status != 1) {
                        $done = $this->postPayment($order_data['out_trade_no'], "Tomatopay");
                        $return = array('ret' => $done['errcode'], 'msg' => $done['errmsg']);
                        echo 'SUCCESS';
                    } else {
                        $return = array('ret' => 0, 'msg' => '交易已完成');
                        echo 'ERROR';
                    }
                } else {
                    echo '验证失败';
                    $return = array('ret' => 0, 'msg' => '验证失败');
                }
                return json_encode($return, JSON_UNESCAPED_UNICODE);
            case ('materialpay'):
                $materialpay = new MaterialPay(Config::get('materialpay_secret'));
