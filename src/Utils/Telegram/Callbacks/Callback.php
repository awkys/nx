...
use App\Models\{LoginIp, Node, Ip, InviteCode, Payback, User, UserSubscribeLog};
use App\Services\Config;
use App\Utils\{Telegram, Tools, QQWry, Telegram\Reply, Telegram\TelegramTools};
...
//        file_put_contents(BASE_PATH . '/storage/crisp.log', $this->CallbackData."\r\n", FILE_APPEND);
        ...
        switch (true) {
            case (strpos($this->CallbackData, 'user.') === 0):
                ...
                $this->userCallback();
                break;
            case (strpos($this->CallbackData, 'crisp.') === 0):
                // crisp相关
                $this->crispCallback();
                break;
            default:
    ...
    public function crispCallback()
    {
        $crisp = new CrispService();
        $call = explode('.', $this->CallbackData);
        switch ($call[1]) {
            case "logs":
                $logs = $crisp->getUserMessageLogs($call[2]);
                $text = "消息记录" . PHP_EOL;
                foreach ($logs["data"] as $log) {
                    $timestamp = substr($log['timestamp'], 0, 10);
                    $text .= '`' . date('Y-m-d H:i:s', $timestamp) . '`' . PHP_EOL;
                    $text .= $log['user']['nickname'] . ':*' . $log['content'] . '*' . PHP_EOL;
                }
                $keyboard = [
                    [
                        [
                            'text' => '标记已完成',
                            'callback_data' => "crisp.success." . $call[2]
                        ]
                    ]
                ];
                ...
                break;
            case "success":
                $result = $crisp->setState($call[2], 'resolved');
                Telegram::Send("消息[" . $call[2] . "],标记成功", $_ENV['telegram_admin_id']);
                break;
            case "user_info":
                $message = $crisp->getUserMessageInfo($call[2]);
                $user = User::query()->where('id', $message['data']['meta']['data']['ID'])->first();
                $text = "用户ID：" . $user->id . PHP_EOL;
                $text .= "用户名：" . $user->user_name . PHP_EOL;
                $text .= "用户邮箱：" . $user->email . PHP_EOL;
                $text .= "注册时间：" . $user->reg_date . PHP_EOL;
                $text .= Reply::getUserInfo($user);
                $text .= PHP_EOL;
                $text .= '流量重置时间：' . $user->valid_use_loop();
                Telegram::Send($text, $_ENV['telegram_admin_id']);
                break;
            default:
                Telegram::Send("没消息", $_ENV['telegram_admin_id']);
        }
    }
