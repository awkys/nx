...
class THeadPay extends AbstractPayment
{
    ...
    public function pay($order)
    {
        $params = [
            ...
            'total_fee' => (string)($order['total_fee'] * 100),
            'notify_url' => rtrim($_ENV['baseUrl'], '/') . '/payment/notify/theadpay',
            'return_url' => rtrim($_ENV['baseUrl'], '/') . "/user/payment/return?tradeno={$order['trade_no']}",
        ];
    ...
    public function MetronPay($type, $price, $buyshop, $paylist_id = 0)
    {
        ...
        $type = 'qrcode';
        $url = $params['code_url'];
        if ($this->isMobile() || (isset($_ENV['theadpay_display']) && $_ENV['theadpay_display'] === 'redirect')) {
            $type = 'url';
